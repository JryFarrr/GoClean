...
- 📤 **Import dari Excel** - Upload file .xlsx/.xls untuk import data TPS secara massal
- 📥 **Download Template** - Template Excel dengan format dan contoh yang benar
...
- 🏪 **Manajemen TPS** - Kelola data lokasi TPS di seluruh Surabaya
- ➕ **Tambah Lokasi TPS** - Tambah TPS baru dengan peta interaktif
- 📤 **Import Excel TPS** - Import data TPS massal dari file Excel
- 🗑️ **Hapus Lokasi TPS** - Hapus lokasi TPS dengan konfirmasi keamanan
...
│   │   │   ├── login/page.tsx     # Admin login page (khusus)
│   │   │   ├── register/page.tsx  # Admin registration
│   │   │   ├── tps/page.tsx       # ✨ Manajemen lokasi TPS (Tambah/Hapus/Import)
│   │   │   └── users/page.tsx     # User management
│   │   ├── 📂 api/                # API Routes
│   │   │   ├── auth/              # Authentication (NextAuth, Register)
│   │   │   ├── admin/             # Admin (Stats, Users, Register)
│   │   │   │   ├── register/      # Admin registration endpoint
│   │   │   │   ├── stats/         # System statistics
│   │   │   │   ├── users/         # User management API
│   │   │   │   └── tps-locations/ # ✨ TPS location CRUD
│   │   │   │       ├── route.ts   # GET (list), POST (create), DELETE (remove)
│   │   │   │       └── import/    # ✨ Import TPS dari Excel
│   │   │   ├── pickups/           # Pickup CRUD endpoints
│   │   │   ├── tps/               # TPS profile & prices
│   │   │   ├── tps-locations/     # TPS locations public API
...
│   │   │   ├── new/               # ✨ Create pickup (dengan pilihan kecamatan)
...
│   │   ├── TPSLocationPicker.tsx  # ✨ Peta pemilih lokasi TPS (admin)
│   │   └── WasteItemSelector.tsx  # Waste type selector
│   ├── 📂 lib/                    # Utility libraries
│   │   ├── auth.ts                # NextAuth configuration
│   │   ├── prisma.ts              # Prisma client
│   │   ├── store.ts               # Zustand store
│   │   ├── surabayaKecamatan.ts   # ✨ Daftar 31 kecamatan Surabaya
...
│       ├── add_tps_locations.sql  # ✨ TPS locations table
│       └── add_gis_layers.sql     # ✨ GIS layers (opsional)
├── 📂 public/
...
├── 📄 README.md                   # This file
├── 📄 DATABASE-STRUCTURE.md       # Dokumentasi database
├── 📄 DATABASE-GIS-STRUCTURE.md   # Dokumentasi GIS layers
├── 📄 FITUR-TAMBAH-LOKASI-TPS.md  # ✨ Panduan tambah TPS
├── 📄 FITUR-HAPUS-LOKASI.md       # ✨ Panduan hapus lokasi pickup
├── 📄 PANDUAN-IMPORT-TPS.md       # ✨ Panduan import TPS dari Excel
└── 📄 SETUP-DATABASE-GIS.md       # Setup GIS database
```
...
- `TPSLocationPicker.tsx` - **✨ Peta picker lokasi TPS untuk admin** (dengan search & drag marker)
- `Navbar.tsx` - Navigation bar dengan role-based menu
- `providers.tsx` - Context providers (NextAuth, Toaster)

#### **src/lib/** - Business Logic
- `auth.ts` - Konfigurasi NextAuth & role-based access
- `prisma.ts` - Database client singleton
- `store.ts` - Global state management
- `surabayaKecamatan.ts` - **✨ Daftar 31 kecamatan Surabaya** (const array)
- `tpsLocations.ts` - (Deprecated - sekarang menggunakan database TPSLocation)
- `utils.ts` - Helper functions

#### **prisma/** - Database Layer
- `schema.prisma` - 13 tables (8 core + 5 GIS opsional)
  - **Core:** User, TPSProfile, PickupRequest, WasteItem, Transaction, Notification, WastePrice
  - **Master Data:** TPSLocation (✨ Baru)
  - **GIS (Opsional):** Kategori, Kecamatan, ObjekPoint, Jalan, Area
- Migrations untuk SQL Server
  - `add_tps_locations.sql` - ✨ Tabel lokasi TPS
  - `add_gis_layers.sql` - ✨ Layer GIS (Point, Line, Polygon)
...
- `POST /api/admin/tps-locations/import` - ✨ Import TPS dari Excel
...
- 📤 **Import Excel** - Upload file .xlsx/.xls untuk import massal
- 📥 **Download Template** - Template Excel dengan format yang benar
